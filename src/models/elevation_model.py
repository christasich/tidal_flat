...
def apply_lin_rates(init_elev, time_arr, org_rate, comp_rate, sub_rate):
    """
    ...
    """
    lin_arr = time_arr * (org_rate - comp_rate - sub_rate)
    elevs = init_elev + lin_arr
...
def simulate_elevation(
    ...
    def aggrade(t, init_vals, *args):
        ...
        delta_conc = (
            -(settle_rate * init_conc) / depth
            - H / depth * (init_conc - bound_conc) * tide_deriv
        )
    ...
    index = np.arange(
        0, len(water_heights) * timestep, timestep
    )  # make numeric index in seconds using timestep
    ...
    while True:
        ...
        data_above_platform = remaining_data[
            remaining_data > (elev_ser + min_depth)
        ]  # tide data that is above the platform

        # break when not enough data to make a spline
        if len(data_above_platform) < 4:
            # add final values to results when model ends with a dry platform
        ...
        pos_at_start = data_above_platform.index[
            0
        ]  # index value of start of next inundation
        pos_at_ends = data_above_platform.index[
            np.where(np.diff(data_above_platform.index) != timestep)
        ]  # index values of end of remaining inundations

        # set end position of integration
        if (
            len(pos_at_ends) == 0
        ):  # Set the end position to the end of the data when the last data point ends above the platform.
            end = data_above_platform.index[-1]
        else:
            end = pos_at_ends[
                0
            ]  # Set the end position to the end of the first inundation in the remaining data.
        ...
        if result.status == -1:
            print(
                "Solver failed to integrate from t={} to t={}.".format(
                    t_span[0], t_span[-1]
                )
            )
    ...
    # add initial values to results when model begins with a dry platform
    if results.t[0] != 0:
...
def run_model(
    ...
    for year in range(0, years):
        offset = (
            len(data) * timestep
        ) * year  # offset to be add to tide index and result times depending on the year
    ...
    if verbose is True:
        ...
        print("Vertical accretion:    {:.4f} m".format(((result.y[1][-1] - init_elev) + (comp_rate + sub_rate) * years)))
        print("Subsidence:            {:.4f} m".format(((comp_rate + sub_rate) * years)))
        print("Elevation change:      {:.4f} m".format((result.y[1][-1] - init_elev)))
